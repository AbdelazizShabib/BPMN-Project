...
import { ZoomIn, ZoomOut, Maximize2, Maximize, Minimize, RefreshCw, Info, Play } from 'lucide-react'
...
export default function BpmnViewer() {
  ...
  // ── Scroll sidebar to the active item whenever selectedNode changes ────────
  useEffect(() => {
    if (selectedNode?.id) {
      sidebarItemRefs.current[selectedNode.id]?.scrollIntoView({
        behavior: 'smooth',
        block: 'nearest',
      })
    }
  ...
  // ── SVG layer reordering — make shapes render above connection arrows ─────
  // bpmn-js appends connections after shapes in the SVG DOM, so connections
  // paint on top of shape text.  Moving shapes to the end of their parent
  // group restores the correct stacking order.
  const reorderSvgLayers = () => {
    const svgEl = containerRef.current?.querySelector('svg')
    if (!svgEl) return

    // Try separate layer groups first (some versions of diagram-js use these)
    ...
    if (connLayer && shapeLayer) {
      // Separate layers: ensure connections layer precedes shapes layer
      baseGroup.insertBefore(connLayer, shapeLayer)
    } else {
      // Single mixed layer: move every shape element to the end
      children
        .filter(c => c.classList.contains('djs-shape'))
        .forEach(shape => baseGroup.appendChild(shape))
    }
  }
  ...
  // ── Sidebar-triggered node selection ──────────────────────────────────────
  const selectNode = id => {
    ...
    // Pan diagram to centre on the element
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <Header
        ...
        subtitle="Full Scrum framework — 35 nodes, 8 decision gateways, multiple feedback loops"
      />

      <main className="flex-1 p-4 lg:p-6 flex flex-col gap-4 overflow-hidden">
        {/* Info banner */}
        <div className="flex items-start gap-3 p-3 rounded-xl bg-blue-950/30 border border-blue-800/30 text-xs text-blue-300 flex-shrink-0">
          <Info size={14} className="flex-shrink-0 mt-0.5" />
          <p>Click any element in the diagram or the list to see details. Use the sidebar to jump to a step — the diagram will pan to it automatically.</p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-4 flex-1 min-h-0">
              ...
              <div className="absolute bottom-4 left-4 right-16 z-10 bg-slate-900/95 border border-slate-700 rounded-xl p-4 shadow-xl backdrop-blur max-w-lg">
                <div className="flex items-start gap-3">
                  ...
                  <button
                    ...
                    className="text-slate-500 hover:text-slate-300 flex-shrink-0 text-xs ml-auto"
                  >
                    ✕
                  </button>
                </div>
          ...
          {/* Sidebar — scrollable list of process steps */}
          <div className="w-full lg:w-72 flex-shrink-0 flex flex-col gap-2 overflow-y-auto lg:max-h-full">
            {groups.map(group => (
              <div
                key={group.label}
                className="bg-slate-800/40 border border-slate-700/40 rounded-xl overflow-hidden flex-shrink-0"
              >
